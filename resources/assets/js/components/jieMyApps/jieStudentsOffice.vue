<template>
  <div class="px-4 py-3">
               <v-layout row wrap>
               <span style="font-size:16px" class="font-weight-bold"><span  style="font-size:18px" class="blue--text">My subjects</span> </span>
               <v-spacer></v-spacer>
               <v-dialog
                  persistent
                  v-model="dialog1"
...
export default {  
   ...
   computed: {
      ...
      mySubject() {
         var getdata = this.$store.getters.acceptStudents
         var filtdata = _.find(getdata,['email',_.toLower(this.userFData.email)])
         console.log('test',filtdata)
         console.log('test1',_.toLower(this.userFData.email))
         console.log('test2',getdata)
         if(filtdata) {
            return filtdata.subjects
         }
   ...
   created() {
      ...
      order.on("value",  function(snapshot) {
       ...
       this.$store.commit('getPrograms')
       this.$store.commit('getFaculties')
