      ...
      <v-layout row wrap>
         <v-flex v-if="listprograms.length == 0" xs12>
            <v-card class="pa-5 mx-5">
               <h3 class="text-xs-center">No Students available yet</h3>
            </v-card>
         </v-flex>
...
<script>
   ...
   methods: {
      viewItem(data) {
         ...
         let subjectsOffer = []
         // _.forEach(data.subjects, function(value, key) {
         //    var getData = db.ref('CKCMDATA/subject/')
         //    var filt = _.find(getData, ['keyIndex', value.keyIndex])
         //    subjectsOffer.push({
         //       getData
         //    })
         // })
   ...
   computed: {
      ...
      listprograms() {
         ...
         let items = []
         let data = this.$store.getters.myprograms
         let dataFaculty = this.$store.getters.acceptStudents
